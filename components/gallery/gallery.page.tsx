...
export default function GalleryPage({ items }: GalleryPageProps) {
  ...
  const categories = items.reduce((acc: { id: string; label: string }[], item) => {
    item.categories.split(";").filter((v): v is string => !!v).forEach((category) => {
      if (!acc.some((cat) => cat.id === category)) {
        acc.push({ id: category, label: category });
      }
    });
    return acc;
  }, []);

  categories.push({ id: 'todas', label: 'Todas' },)

  const filteredImages =
    selectedCategory === 'todas'
      ? items
      : items.filter((img) => img.categories.includes(selectedCategory));
            ...
            <div className="mt-6 space-y-3">
              ...
              <div className="flex items-center justify-between pt-2">
                <div className={ 'flex items-center gap-2' }>
                  {
                    currentImage.categories.split(";").map((category, index) => (
                      <span
                        key={ index + category }
